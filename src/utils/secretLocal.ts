...
export function upsertSecretLocalKey(key: string) {
...
export function commentSecretLocalKey(key: string) {
  ensureFileDir(SECRET_LOCAL_FILE)
...
export function upsertSecretLocalKeyValue(key: string, value: string) {
  ensureFileDir(SECRET_LOCAL_FILE)
...
export function listCommentedSecretsWithValue(): Array<{ key: string; value: string }> {
  ensureFileDir(SECRET_LOCAL_FILE)
  ...
  for (const line of lines) {
    ...
    if (!key) continue
    if (value.trim().length === 0) continue // value non deve essere solo spazi
  ...
  for (const item of out) map.set(item.key, item.value)
...
export function uncommentAndSetSecretLocal(key: string, value: string) {
  ensureFileDir(SECRET_LOCAL_FILE)
